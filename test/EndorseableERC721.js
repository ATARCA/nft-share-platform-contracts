...
describe("Endorsable ERC721 contract", function() {
  ...
  describe("Token minting (endorsing)", function() {
    ...
    it("Should be able to endorse existing contribution", async function() {
      const e_minting = await instanceEndorsableTokenContract.connect(addr2)["share(uint256)"](s_tokenId)
      //.mint(s_tokenId)
      ...
      expect(e_minting).to.emit(instanceEndorsableTokenContract, "Share").withArgs(addr2.address, addr1.address, 0, s_tokenId)
    })

    it("Should not be able to endorse token that doesn't exist", async function() {
      await expect(instanceEndorsableTokenContract.connect(owner)["share(uint256)"](002)).to.be.revertedWith("Contribution token must exist")
    })

    it("Should not be able to endorse same contribution twice from same address", async function() {
      //Endorse once
      const e_minting = await instanceEndorsableTokenContract.connect(addr1)["share(uint256)"](s_tokenId)
      ...
      expect(e_minting).to.emit(instanceEndorsableTokenContract, "Share").withArgs(addr1.address, addr1.address, 0, s_tokenId)
      //Try to endorse again
      await expect(instanceEndorsableTokenContract.connect(addr1)["share(uint256)"](s_tokenId)).to.be.revertedWith("Contributions cannot be endorsed twice")
    })
    ...
    it("Should be able to endorse same contribution from different wallets", async function() {      
      ...
      //Endorse again
      const e_minting_w2 = await instanceEndorsableTokenContract.connect(addr2)["share(uint256)"](s_tokenId)
      ...
      expect(e_minting_w2).to.emit(instanceEndorsableTokenContract, "Share").withArgs(addr2.address, addr1.address, 1, s_tokenId)
    })

    it("Tokens should not be transferrable", async function() {
      await instanceEndorsableTokenContract.connect(addr1)["share(uint256)"](s_tokenId)
    ...
    it("should be able to endorse even if already has liked a contribution", async function() {
      ...
      await expect(instanceEndorsableTokenContract.connect(addr1)["share(uint256)"](s_tokenId))
    })
