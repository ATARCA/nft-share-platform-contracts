...
describe("Endorsable ERC721 contract", function() {
  ...
  beforeEach(async function() {
    ...
    instanceShareableTokenContract = await upgrades.deployProxy(ShareableTokenContract, ["ShareableToken", "ST", owner.address])
    ...
    instanceEndorsableTokenContract = await upgrades.deployProxy(EndorsableTokenContract, ["EndorseERC721", "ET", owner.address]);
    ...
    await instanceShareableTokenContract.mint(addr1.address, categoryName)
    await instanceShareableTokenContract.mint(addr2.address, categoryName)
  })
  ...
  describe("Token minting (endorsing)", function() {
    ...
    it("Should be able to endorse existing contribution", async function() {
      ...
      expect(e_minting).to.emit(instanceEndorsableTokenContract, "Transfer").withArgs(ethers.constants.AddressZero, addr2.address, s_tokenId)
    ...
    it("Should not be able to endorse same contribution twice from same address", async function() {
      ...
      expect(e_minting).to.emit(instanceEndorsableTokenContract, "Transfer").withArgs(ethers.constants.AddressZero, addr1.address, 0)
    ...
    it("Should be able to endorse same contribution from different wallets", async function() {      
      ...
      expect(e_minting_w2).to.emit(instanceEndorsableTokenContract, "Transfer").withArgs(ethers.constants.AddressZero, addr2.address, 1)
